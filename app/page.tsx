...
function Logo() {
  return <Image id="logo" src="/logo.png" alt="Logo" width={500} height={83.9} />;
}
...
function TextArea(props: TextAreaProps) {
  ...
  React.useLayoutEffect(() => {
    if (ref.current != null) {
      ...
      ref.current.style.height = `${ref.current.scrollHeight + props.borderwidth * 2}px`;
    }
...
function App({ initialMemos }: AppProps) {
  ...
  const handleChange = (key: number) => async (e: React.ChangeEvent<HTMLTextAreaElement>) => {
  ...
  const handleDelete = (key: number) => async (_: React.MouseEvent<HTMLAnchorElement>) => {
    dispatch(["DELETE", key]);
      ...
      <div id="result" className="conainer">
        {memos.map(([key, content]) => (
          <div key={key} className="result-memo">
            <TextArea borderwidth={8} value={content} onChange={handleChange(key)} />
